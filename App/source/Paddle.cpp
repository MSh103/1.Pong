...
void Paddle::Update(Ball& ball, GameState& state)
{
...
void Paddle::UpdateLocal()
{
	...
	{
		if (m_PaddlePosition.y > m_PaddingY && IsKeyDown(KEY_UP))
			m_PaddleVelocity.y = -m_Speed;
		else if ((m_PaddlePosition.y + m_PaddleDimensions.y) < (m_ScreenHeight - m_PaddingY) && IsKeyDown(KEY_DOWN))
			m_PaddleVelocity.y = m_Speed;
		else
			m_PaddleVelocity.y = 0;
	}
